"""task2: Develop a simple image encryption tool using pixel manipulation. support operations like swapping pixel vlaue or appling abasic mathematical operation to each pixel."""
...
def swap_pixels(image):
    ...
    for y in range(0, height - 1, 2):
        for x in range(0, width - 1, 2):
            img_array[y, x], img_array[y + 1, x + 1] = img_array[y + 1, x + 1], img_array[y, x].copy()
            img_array[y, x + 1], img_array[y + 1, x] = img_array[y + 1, x], img_array[y, x + 1].copy()
